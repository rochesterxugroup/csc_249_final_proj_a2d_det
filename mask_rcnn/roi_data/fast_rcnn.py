...
import mask_rcnn.roi_data.keypoint_rcnn
import mask_rcnn.roi_data.mask_rcnn
...
import mask_rcnn.roi_data as roi_data
...
def get_fast_rcnn_blob_names(is_training=True):
    ...
    if is_training:
        ...
        blob_names += ['action_labels_int32']
    if is_training:
        # bbox_targets blob: R bounding-box regression targets with 4
        # targets per class
        blob_names += ['actor_bbox_targets']
        blob_names += ['action_bbox_targets']
        # bbox_inside_weights blob: At most 4 targets per roi are active
        # this binary vector sepcifies the subset of active targets
        blob_names += ['actor_bbox_inside_weights']
        blob_names += ['actor_bbox_outside_weights']
        blob_names += ['action_bbox_inside_weights']
        blob_names += ['action_bbox_outside_weights']
    ...
    if is_training:
        blob_names += ['kept_overlaps']
        blob_names += ['kept_box_to_gt_ind_map']
    return blob_names
...
def _sample_rois(roidb, im_scale, batch_idx):
    ...
    actor_sampled_labels[fg_rois_per_this_image:] = 0  # Label bg RoIs with class 0
    ...
    if 'actor_bbox_targets' not in roidb:
        ...
        actor_bbox_targets = _compute_targets(
            sampled_boxes, gt_boxes[gt_assignments, :], actor_sampled_labels)
        actor_bbox_targets, actor_bbox_inside_weights = _expand_bbox_targets(actor_bbox_targets, cfg.MODEL.NUM_ACTOR_CLASSES)
    else:
        actor_bbox_targets, actor_bbox_inside_weights = _expand_bbox_targets(
            roidb['actor_bbox_targets'][keep_inds, :], cfg.MODEL.NUM_ACTOR_CLASSES)

    if 'action_bbox_targets' not in roidb:
        gt_inds = np.where(roidb['action_gt_classes'] > 0)[0]
        gt_boxes = roidb['boxes'][gt_inds, :]
        gt_assignments = gt_inds[roidb['box_to_gt_ind_map'][keep_inds]]
        action_bbox_targets = _compute_targets(
            sampled_boxes, gt_boxes[gt_assignments, :], actor_sampled_labels)
        action_bbox_targets, action_bbox_inside_weights = _expand_bbox_targets(action_bbox_targets, cfg.MODEL.NUM_ACTION_CLASSES)
    else:
        action_bbox_targets, action_bbox_inside_weights = _expand_bbox_targets(
            roidb['action_bbox_targets'][keep_inds, :],  cfg.MODEL.NUM_ACTION_CLASSES)
    ...
    blob_dict = dict(
        labels_int32=actor_sampled_labels.astype(np.int32, copy=False),
        action_labels_int32=action_sampled_labels.astype(np.int32, copy=False),
        rois=sampled_rois,

...
import mask_rcnn.roi_data.data_utils as data_utils
...
def add_rpn_blobs(blobs, im_scales, roidb):
    ...
    for im_i, entry in enumerate(roidb):
        ...
        gt_inds = np.where(
            (entry['actor_gt_classes'] > 0) & (entry['is_crowd'] == 0)
        )[0]
        ...
        gt_boxes[:, 5] = entry['actor_gt_classes'][gt_inds]
    ...
    valid_keys = [
        'has_visible_keypoints', 'boxes', 'segms', 'seg_areas', 'actor_gt_classes', 'action_gt_classes',
        'actor_gt_overlaps', 'action_gt_overlaps', 'is_crowd', 'box_to_gt_ind_map', 'gt_keypoints'
    ]

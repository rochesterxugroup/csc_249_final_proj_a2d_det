...
import mask_rcnn.utils.segms as segm_utils
...
def add_A2D_mask_rcnn_blobs(blobs, sampled_boxes, roidb, im_scale, batch_idx):
    """Add Mask R-CNN specific blobs to the input blob dictionary."""
    # Prepare the mask targets by associating one gt mask to each training roi
    # that has a fg (non-bg) class label.
    M = cfg.MRCNN.RESOLUTION
    ...
    # matrix shape: number of gt_indices \times 4
    boxes_from_polys = [roidb['boxes'][i] for i in polys_gt_inds]  # use boxes directly
    fg_inds = np.where(blobs['labels_int32'] > 0)[0]
    roi_has_mask = blobs['labels_int32'].copy()
    roi_has_mask[roi_has_mask > 0] = 1

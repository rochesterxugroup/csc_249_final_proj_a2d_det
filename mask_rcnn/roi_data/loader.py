...
from mask_rcnn.roi_data.minibatch import get_minibatch
import mask_rcnn.utils.blob as blob_utils


class RoiDataLoader(data.Dataset):
    def __init__(self, roidb, frame_root, flow_root, training=True):
        self._roidb = roidb
    ...
    def __getitem__(self, index_tuple):
        ...
        if self._roidb[index]['need_crop']:
            ...
            if len(valid_inds) < len(boxes):
                for key in ['boxes', 'actor_gt_classes', 'action_gt_classes',
                            'seg_areas', 'actor_gt_overlaps', 'action_gt_overlaps', 'is_crowd',
    ...
    def crop_segment(self, blobs, ratio):
        data_height, data_width = map(int, blobs['im_info'][:2])
        boxes = blobs['roidb'][0]['boxes']
        if ratio < 1:  # width << height, crop height
            size_crop = math.ceil(data_width / ratio)  # size after crop
            ...
            if min_y == 0:
                ...
                if (box_region - size_crop) < 0:
                    ...
                    y_s = min_y if y_s_add == 0 else \
                        npr.choice(range(min_y, min_y + y_s_add + 1))
            # Crop the image
            blobs['data'] = blobs['data'][:, :, y_s:(y_s + size_crop), :,]  # add batch dimension
            blobs['flow'] = blobs['flow'][:, :, y_s:(y_s + size_crop), :,]  # add cropping flow
            # Update im_info
            blobs['im_info'][0] = size_crop
            # Shift and clamp boxes ground truth
            boxes[:, 1] -= y_s
            ...
            if min_x == 0:
                ...
                if (box_region - size_crop) < 0:
                    ...
                    x_s = min_x if x_s_add == 0 else \
                        npr.choice(range(min_x, min_x + x_s_add + 1))
            # Crop the segment
            blobs['data'] = blobs['data'][:, :, :, x_s:(x_s + size_crop)]  # add batch dimension
            blobs['flow'] = blobs['flow'][:, :, :, x_s:(x_s + size_crop)]  # add cropping flow
            # Update im_info
            blobs['im_info'][1] = size_crop
            # Shift and clamp boxes ground truth
            boxes[:, 0] -= x_s
...
def collate_minibatch(list_of_blobs):
    ...
    for i in range(0, len(list_of_blobs), cfg.TRAIN.IMS_PER_BATCH):
        ...
        # Pad image data
        mini_list = pad_seq_data(mini_list)
        # Pad flow data
        mini_list = pad_flow_data(mini_list)
...
def pad_seq_data(list_of_blobs):
    max_shape = blob_utils.get_max_shape([blobs['data'].shape[2:] for blobs in list_of_blobs])
    output_list = []
    for blob in list_of_blobs:
        data_padded = np.zeros((cfg.A2D.SEGMENT_LENGTH, 3, max_shape[0], max_shape[1]), dtype=np.float32)
        _, _, h, w = blob['data'].shape
        data_padded[:, :, :h, :w] = blob['data']
        blob['data'] = data_padded
        output_list.append(blob)
    return output_list


def pad_flow_data(list_of_blobs):
    max_shape = blob_utils.get_max_shape([blobs['flow'].shape[2:] for blobs in list_of_blobs])
    output_list = []
    for blob in list_of_blobs:
        t, c, h, w = blob['flow'].shape
        data_padded = np.zeros((cfg.A2D.SEGMENT_LENGTH, c, max_shape[0], max_shape[1]), dtype=np.float32)
        data_padded[:, :, :h, :w] = blob['flow']
        blob['flow'] = data_padded
        output_list.append(blob)
    return output_list

...
import mask_rcnn.utils.env as envu
...
class JsonDataset(object):
    ...
    def valid_cached_keys(self):
        ...
        """
        keys = ['boxes', 'segms', 'actor_gt_classes', 'action_gt_classes', 'seg_areas',
                'actor_gt_overlaps', 'action_gt_overlaps',
...
def _merge_proposal_boxes_into_roidb(roidb, box_list):
    ...
    for i, entry in enumerate(roidb):
        ...
        actor_gt_overlaps = np.zeros(
            (num_boxes, entry['actor_gt_overlaps'].shape[1]),
            dtype=entry['actor_gt_overlaps'].dtype
        )
        action_gt_overlaps = np.zeros(
            (num_boxes, entry['action_gt_overlaps'].shape[1]),
            dtype=entry['action_gt_overlaps'].dtype
        )
        ...
        # assume that actor-action must be foreground at the same time
        gt_inds = np.where(entry['actor_gt_classes'] > 0)[0]
        if len(gt_inds) > 0:
            gt_boxes = entry['boxes'][gt_inds, :]
            actor_gt_classes = entry['actor_gt_classes'][gt_inds]
            action_gt_classes = entry['action_gt_classes'][gt_inds]
            ...
            # for each proposal, which index of class id has the highest overlap
            # (if all overlaps are zeros, first index of class id will be returned)
            argmaxes = proposal_to_gt_overlaps.argmax(axis=1)
            # Amount of that overlap
            maxes = proposal_to_gt_overlaps.max(axis=1)
            # Those boxes with non-zero overlap with gt boxes
            # will filter out proposals that has no overlap with any ground truth box
            I = np.where(maxes > 0)[0]
            # Record max overlaps with the class of the appropriate gt box
            actor_gt_overlaps[I, actor_gt_classes[argmaxes[I]]] = maxes[I]
            action_gt_overlaps[I, action_gt_classes[argmaxes[I]]] = maxes[I]
        ...
        entry['actor_gt_classes'] = np.append(
            entry['actor_gt_classes'],
            np.zeros((num_boxes), dtype=entry['actor_gt_classes'].dtype)
        )
        entry['action_gt_classes'] = np.append(
            entry['action_gt_classes'],
            np.zeros((num_boxes), dtype=entry['action_gt_classes'].dtype)
        )
        entry['seg_areas'] = np.append(
            entry['seg_areas'],
            np.zeros((num_boxes), dtype=entry['seg_areas'].dtype)
        )
        entry['actor_gt_overlaps'] = np.append(
            entry['actor_gt_overlaps'].toarray(), actor_gt_overlaps, axis=0
        )
        entry['actor_gt_overlaps'] = scipy.sparse.csr_matrix(entry['actor_gt_overlaps'])

        entry['action_gt_overlaps'] = np.append(
            entry['action_gt_overlaps'].toarray(), action_gt_overlaps, axis=0
        )
        entry['action_gt_overlaps'] = scipy.sparse.csr_matrix(entry['action_gt_overlaps'])
...
def _add_class_assignments(roidb):
    ...
    """
    for entry in roidb:
        # for each roi, class id with highest overlap will be stored with the overlap: 0.0 to 1.0
        actor_gt_overlaps = entry['actor_gt_overlaps'].toarray()
        # max overlap with gt over classes (columns)

        # it's like remove all the zeros in actor_gt_overlaps
        actor_max_overlaps = actor_gt_overlaps.max(axis=1)
        # gt class that had the max overlap
        actor_max_classes = actor_gt_overlaps.argmax(axis=1)

        action_gt_overlaps = entry['action_gt_overlaps'].toarray()
        # max overlap with gt over classes (columns)
        action_max_overlaps = action_gt_overlaps.max(axis=1)
        # gt class that had the max overlap
        action_max_classes = action_gt_overlaps.argmax(axis=1)

        entry['actor_max_classes'] = actor_max_classes
        entry['actor_max_overlaps'] = actor_max_overlaps

        entry['action_max_classes'] = action_max_classes
        entry['action_max_overlaps'] = action_max_overlaps
        # sanity checks
        # if max overlap is 0, the class must be background (class 0)
        actor_zero_inds = np.where(actor_max_overlaps == 0)[0]
        assert all(actor_max_classes[actor_zero_inds] == 0)

        # sanity checks
        # if max overlap is 0, the class must be background (class 0)
        action_zero_inds = np.where(action_max_overlaps == 0)[0]
        assert all(action_max_classes[action_zero_inds] == 0)

        # if max overlap > 0, the class must be a fg class (not class 0)
        actor_nonzero_inds = np.where(actor_max_overlaps > 0)[0]
        assert all(actor_max_classes[actor_nonzero_inds] != 0)

        # if max overlap > 0, the class must be a fg class (not class 0)
        action_nonzero_inds = np.where(action_max_overlaps > 0)[0]
        assert all(action_max_classes[action_nonzero_inds] != 0)

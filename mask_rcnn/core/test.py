...
import mask_rcnn.utils.boxes as box_utils
import mask_rcnn.utils.image as image_utils

# from mask_rcnn.dataset.utils import Timer
# from dataset import utils as box_utils, utils as blob_utils, utils as fpn_utils, utils as image_utils, \
#     utils as keypoint_utils

import mask_rcnn.utils.blob as blob_utils
import mask_rcnn.utils.fpn as fpn_utils
import mask_rcnn.utils.keypoints as keypoint_utils


def im_just_det_cls(model, im, vid_frm_idx, box_proposals=None, timers=None):
    ...
    actor_scores, action_scores, boxes, im_scale, blob_conv = im_detect_bbox(
            model, im, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, vid_frm_idx, box_proposals)
    ...
    actor_scores, action_scores, boxes, cls_boxes = box_results_with_nms_and_limit(actor_scores, action_scores, boxes)
    timers['misc_bbox'].toc()

    pred_box_results = []
    for actor_cls_id, boxes_conf_cls in enumerate(cls_boxes):
        for box_conf_cls in boxes_conf_cls:
            pred_box_results.append(np.insert(box_conf_cls, 6, actor_cls_id))

    return np.array(pred_box_results)


def im_detect_all(model, im, vid_frm_idx, box_proposals=None, timers=None):
    """Process the outputs of model for testing
    Args:
      model: the network module
      im_data: Pytorch variable. Input batch to the model.
...
def im_detect_bbox(model, im, target_scale, target_max_size, vid_frm_idx, boxes=None):
    """Prepare the bbox for testing"""

    inputs, im_scale = _get_blobs(im, boxes, target_scale, target_max_size, vid_frm_idx)
    ...
    inputs['flow'] = [torch.from_numpy(inputs['flow'])]

    # split temporal dimension
    data_b, data_c, data_h, data_w = inputs['data'][0].size()
    flow_b, flow_c, flow_h, flow_w = inputs['flow'][0].size()
    inputs['data'][0] = inputs['data'][0].view(1, cfg.A2D.SEGMENT_LENGTH, data_c, data_h, data_w)
    inputs['flow'][0] = inputs['flow'][0].view(1, cfg.A2D.SEGMENT_LENGTH, flow_c, flow_h, flow_w)
    ...
    actor_scores = return_dict['actor_cls_score'].data.cpu().numpy().squeeze()
    action_scores = return_dict['action_cls_score'].data.cpu().numpy().squeeze()
    # In case there is 1 proposal
    actor_scores = actor_scores.reshape([-1, actor_scores.shape[-1]])
    ...
    if cfg.TEST.BBOX_REG:
        ...
        if cfg.MODEL.CLS_AGNOSTIC_BBOX_REG:
            # TODO: consider action scores
            pred_boxes = np.tile(pred_boxes, (1, actor_scores.shape[1]))
    else:
        # Simply repeat the boxes, once for each class
        # TODO: consider action scores
        pred_boxes = np.tile(boxes, (1, actor_scores.shape[1]))

    if cfg.DEDUP_BOXES > 0 and not cfg.MODEL.FASTER_RCNN:
        # Map scores and predictions back to the original set of boxes
        # TODO: consider action scores
        actor_scores = actor_scores[inv_index, :]
        pred_boxes = pred_boxes[inv_index, :]

    return actor_scores, action_scores, pred_boxes, im_scale, return_dict['blob_conv']
...
def im_detect_mask(model, im_scale, boxes, blob_conv):
    ...
    if cfg.MRCNN.CLS_SPECIFIC_MASK:
        pred_masks = pred_masks.reshape([-1, cfg.MODEL.NUM_ACTOR_CLASSES, M, M])
    else:
...
def box_results_with_nms_and_limit(actor_scores, action_scores, boxes):  # NOTE: support single-batch
    ...
    action_scores_max_score_class = np.argmax(action_scores, 1)  # 10 is maximum value
    action_scores_max_score = np.max(action_scores, 1)
    # num_classes = cfg.MODEL.NUM_CLASSES
    actor_num_classes = cfg.MODEL.NUM_ACTOR_CLASSES
    action_num_classes = cfg.MODEL.NUM_ACTION_CLASSES
    # TODO: add action
    cls_boxes = [[] for _ in range(actor_num_classes)]
    # Apply threshold on detection probabilities and apply NMS
    # Skip j = 0, because it's the background class
    # TODO: add action
    for j in range(1, actor_num_classes):
        inds = np.where(actor_scores[:, j] > cfg.TEST.SCORE_THRESH)[0]
        actor_scores_j = actor_scores[inds, j]
        action_scores_j = action_scores_max_score[inds]
        action_cls_j = action_scores_max_score_class[inds]
        boxes_j = boxes[inds, j * 4:(j + 1) * 4]
        dets_j = np.hstack((boxes_j, actor_scores_j[:, np.newaxis])).astype(np.float32, copy=False)
        dets_j_with_action = np.hstack((boxes_j,
                                        actor_scores_j[:, np.newaxis],
                                        action_scores_j[:, np.newaxis],
                                        action_cls_j[:, np.newaxis])).astype(np.float32, copy=False)
        if cfg.TEST.SOFT_NMS.ENABLED:
            ...
            nms_dets_with_action = dets_j_with_action[keep, :]
        ...
        assert nms_dets_with_action is not None
        cls_boxes[j] = nms_dets_with_action

    # Limit to max_per_image detections **over all classes**
    if cfg.TEST.DETECTIONS_PER_IM > 0:
        # TODO: add action
        image_scores = np.hstack(
            [cls_boxes[j][:, -3] for j in range(1, actor_num_classes)]
        )
        if len(image_scores) > cfg.TEST.DETECTIONS_PER_IM:
            image_thresh = np.sort(image_scores)[-cfg.TEST.DETECTIONS_PER_IM]
            # TODO: add action
            for j in range(1, actor_num_classes):
                keep = np.where(cls_boxes[j][:, -3] >= image_thresh)[0]
                cls_boxes[j] = cls_boxes[j][keep, :]

    # TODO: add action
    im_results = np.vstack([cls_boxes[j] for j in range(1, actor_num_classes)])
    boxes = im_results[:, :-3]
    actor_scores = im_results[:, -3]
    action_scores = im_results[:, -2]
    return actor_scores, action_scores, boxes, cls_boxes


def segm_results(cls_boxes, masks, ref_boxes, im_h, im_w):
    num_classes = cfg.MODEL.NUM_ACTOR_CLASSES
...
def _get_blobs(im, rois, target_scale, target_max_size, vid_frm_idx):
    """Convert an image and RoIs within that image into network inputs."""
    blobs = {}
    blobs['data'], im_scale, blobs['im_info'] = \
        blob_utils.get_seq_blob(im, target_scale, target_max_size, vid_frm_idx)
    blobs['flow'] = blob_utils.get_flow_blob(target_scale, target_max_size, vid_frm_idx)
    if rois is not None:

...
class fast_rcnn_outputs(nn.Module):
    def __init__(self, dim_in):
        ...
        self.cls_score = nn.Linear(dim_in, cfg.MODEL.NUM_ACTOR_CLASSES)
        if cfg.MODEL.CLS_AGNOSTIC_BBOX_REG:  # bg and fg
            self.bbox_pred = nn.Linear(dim_in, 4 * 2)
        else:
            self.bbox_pred = nn.Linear(dim_in, 4 * cfg.MODEL.NUM_ACTOR_CLASSES)
...
class fast_rcnn_action_outputs(nn.Module):
    def __init__(self, dim_in):
        super(fast_rcnn_action_outputs, self).__init__()
        self.cls_score = nn.Linear(dim_in, cfg.MODEL.NUM_ACTION_CLASSES)
        self._init_weights()
...
def fast_rcnn_action_losses(cls_score, label_int32):
    device_id = cls_score.get_device()
    rois_label = Variable(torch.from_numpy(label_int32.astype('int64'))).cuda(device_id)
    loss_cls = F.cross_entropy(cls_score, rois_label)
